...
docker build -q -t grn-vae:base .
if ([[ "$(docker images -q grn-vae:base 2> /dev/null)" != "" ]]); then
    echo "Docker container for GRN-VAE is built and tagged as grn-vae:base"
else
    echo "Oops! Unable to build Docker container for GRN-VAE"
fi
